...
import { MOCK_APPOINTMENTS, MOCK_STAFF, MOCK_SERVICES, MOCK_CUSTOMERS } from '../constants';

// Simulate a database
let appointmentsDB: Appointment[] = [...MOCK_APPOINTMENTS.map(a => ({...a, start: new Date(a.start), end: new Date(a.end)}))];

const simulateDelay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

export const fetchAppointments = async (filters?: { date?: Date; staffId?: string; serviceId?: string; status?: AppointmentStatus }): Promise<Appointment[]> => {
  ...
  return appointmentsDB.filter(appt => {
    let match = true;
    if (filters?.date) {
      match = match && appt.start.toDateString() === filters.date.toDateString();
    }
    if (filters?.staffId) {
