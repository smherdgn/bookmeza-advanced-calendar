...
import { Staff, Service, Appointment, AppointmentStatus, ThemeColors, ThemeShadows, Theme } from './types';

export const DEFAULT_TENANT_ID = 'tenant-123';
export const DEFAULT_TENANT_TIMEZONE = 'Europe/Berlin'; // Example

// Note: Names for MOCK_STAFF and MOCK_SERVICES will be translated using i18n in components
// e.g., t(`mock.staff.${staff.id}.name`)
...
export const MOCK_APPOINTMENTS: Appointment[] = [
  {
    ...
    title: 'Consultation with Alice' // Example user-defined title
  },
  {
    id: 'appt-2',
    ...
    tenantId: DEFAULT_TENANT_ID,
    title: 'Check-up for Bob' // Example user-defined title
  },
  {
    id: 'appt-3',
    ...
    status: AppointmentStatus.CONFIRMED,
    tenantId: DEFAULT_TENANT_ID,
    title: 'Therapy Session' // Example user-defined title for a service that might not have one specified by default
  },
   {
    id: 'appt-4',
    ...
    tenantId: DEFAULT_TENANT_ID,
    title: 'Follow-up with Alice' // Example user-defined title
  },
];
...
// APPOINTMENT_STATUS_DISPLAY is removed, will use t(`appointment.status.${statusKey}`)
// CALENDAR_VIEWS is removed, will use t(`calendar.view.${viewKey}`)
